...
template<class T, typedef Float>
class ode
{
public:
...
public:
    ...
    typedef std::vector< array_wrapper<Float> > recording;
    ...
    ode() : _N(T.get_num_states()),
        _state(new Float[_N + 1]),
        _state_rate(new Float[_N + 1]),
        options(_N)
    {}
    ...
    inline recording run(run_options& options)
    {
        ...
        record.reserve(nRecordings);

        memcpy(_state, options.initial_state, _N * sizeof(Float));
        _state[_N] = options.start_time;

        const unsigned copyN = _N + 1u;

        record.emplace_back(_state, copyN);

        while (_state[_N] < options.end_time && T.valid())
        {
            step(this);
            if (_state[_N] > time_record)
            {
                record.emplace_back(_state, copyN);
            }
        }
